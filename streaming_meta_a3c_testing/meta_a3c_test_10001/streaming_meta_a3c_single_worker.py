...
class Worker():
	def __init__(self, env, name, a_size, repeat, trainer, model_path, global_episodes):
		...
		self.summary_writer = tf.summary.FileWriter("meta_a3c_test_" + str(self.number))
	...
	# from the work(): episode_buffer.append([state, a, r, t, d, v])
		...
		with sess.as_default(), sess.graph.as_default():
			...
			while nth_repeat < self.repeat:
				...
				while d == False:
					...
					episode_buffer.append([state, a, r, t, d, v])
				...
				np.savetxt(os.path.join(self.result_path, \
					'meta-A3C_test_' + str(nth_repeat)  + '_actions.txt'), episode_action, fmt='%1d')
				np.savetxt(os.path.join(self.result_path, \
					'meta-A3C_test_' + str(nth_repeat)  + '_qoes.txt'), episode_qoe, fmt='%5.3f')
				np.savetxt(os.path.join(self.result_path, \
					'meta-A3C_test_' + str(nth_repeat)  + '_costs.txt'), episode_cost, fmt='%5.3f')
				np.savetxt(os.path.join(self.result_path, \
					'meta-A3C_test_' + str(nth_repeat)  + '_rewards.txt'), episode_reward, fmt='%5.3f')
				np.savetxt(os.path.join(self.result_path, \
					'meta-A3C_test_' + str(nth_repeat)  + '_rebuffering_time.txt'), episode_rebuffering, fmt='%5.3f')

				nth_repeat += 1
			
			np.savetxt(os.path.join(self.result_path, \
				'meta-A3C_test_total_qoes.txt'), self.episode_qoes, fmt='%5.3f')
			np.savetxt(os.path.join(self.result_path, \
				'meta-A3C_test_total_costs.txt'), self.episode_costs, fmt='%5.3f')
			np.savetxt(os.path.join(self.result_path, \
				'meta-A3C_test_total_rewards.txt'), self.episode_rewards, fmt='%5.3f')
			np.savetxt(os.path.join(self.result_path, \
				'meta-A3C_test_total_rebuffering_count.txt'), self.episode_rebuffering_count, fmt='%2d')