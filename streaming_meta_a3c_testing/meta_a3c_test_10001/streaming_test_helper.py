...
def actions_graph(minutes, repeat, worker_num):
	...
	for i in range(repeat):
		z = genfromtxt(os.path.join(result_path, 'meta-A3C_test_' + str(i)  + '_actions.txt'))
		...
		plt.title('meta-A3C agent action for test_' + str(i) + ' with wifi & LTE bandwidth')
		plt.grid(True)
		plt.savefig(os.path.join(plotting_path, 'meta-A3C_test_' + str(i) + '_action_with_bandwdith.png'))
		plt.close()

		"""
		plt.figure(i * 13, figsize=[15, 10])
		z = genfromtxt(os.path.join(result_path, 'meta-A3C_test_' + str(i)  + '_costs.txt'))
		x = np.arange(0, minutes*60, 4)
		plt.plot(x, z)
		plt.xticks(np.arange(0, minutes*60+4, 4), rotation=45)
		plt.xlabel('testing steps (1 episode == ' + str(minutes*60) +' seconds)')
		plt.ylabel('costs')
		plt.title('meta-A3C test_' + str(i) + ' cost graph')
		plt.grid(True)
		plt.savefig(os.path.join(plotting_path, 'meta-A3C_test_' + str(i)  + '_costs.png'))
		plt.close()

		plt.figure(i * 17, figsize=[15, 10])
		z = genfromtxt(os.path.join(result_path, 'meta-A3C_test_' + str(i)  + '_qoes.txt'))
		x = np.arange(0, minutes*60, 4)
		plt.plot(x, z)
		plt.xticks(np.arange(0, minutes*60+4, 4), rotation=45)
		plt.xlabel('testing steps (1 episode == ' + str(minutes*60) +' seconds)')
		plt.ylabel('qoes')
		plt.title('meta-A3C test_' + str(i) + ' QoE graph')
		plt.grid(True)
		plt.savefig(os.path.join(plotting_path, 'meta-A3C_test_' + str(i)  + '_qoes.png'))
		plt.close()	

		plt.figure(i * 19, figsize=[15, 10])
		z = genfromtxt(os.path.join(result_path, 'meta-A3C_test_' + str(i)  + '_rewards.txt'))
		x = np.arange(0, minutes*60, 4)
		plt.plot(x, z)
		plt.xticks(np.arange(0, minutes*60+4, 4), rotation=45)
		plt.xlabel('testing steps (1 episode == ' + str(minutes*60) +' seconds)')
		plt.ylabel('rewards')
		plt.title('meta-A3C test_' + str(i) + ' reward graph')
		plt.grid(True)
		plt.savefig(os.path.join(plotting_path, 'meta-A3C_test_' + str(i)  + '_rewards.png'))
		plt.close()	
		"""
	return None
